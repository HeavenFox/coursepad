...
import (
	...
	"coursepad/server/common/fb"
	"coursepad/server/common/httpcache"
	...
	"github.com/lib/pq"
	draw2dbase "github.com/llgcode/draw2d"
	draw2d "github.com/llgcode/draw2d/draw2dimg"
...
func drawRect(path *draw2d.GraphicContext, x1, y1, x2, y2 float64) {
	path.MoveTo(x1, y1)
	path.LineTo(x2, y1)
	path.LineTo(x2, y2)
	path.LineTo(x1, y2)
...
func DrawClass(gc *draw2d.GraphicContext, x, y, w, h float64, c color.Color, title, time string) {
	gc.SetFillColor(c)
	drawRect(gc, x, y, x+w, y+h)
	gc.Fill()
...
func generateImage(meetings []singleMeeting) []byte {
	...
	fontData := draw2dbase.FontData{}
	draw2dbase.RegisterFont(fontData, defaultFont)
	...
	drawRect(context, 0, 0, IMAGE_WIDTH, IMAGE_HEIGHT)
	...
	drawRect(context, LEFT_MARGIN, TOP_MARGIN, IMAGE_WIDTH, IMAGE_HEIGHT)
	...
	for y := float64(TOP_MARGIN + STRIPE_TOP_MARGIN); y < IMAGE_HEIGHT; y += 2 * STRIPE_HEIGHT {
		drawRect(context, LEFT_MARGIN, y, IMAGE_WIDTH, y+STRIPE_HEIGHT)

		context.Fill()
	}

	context.SetFontData(fontData)
